            ...
            <>
              {index === 0 ? (
                <Link
                  ...
                  className="group w-2/3 no-underline pr-10"
                >
                  <div key={index} className="mt-8 overflow-hidden">
                    {post.node.featuredImage ? (
                      <div className="w-full h-[636px] overflow-hidden">
                        <Image
                          className="object-cover w-full h-[636px] overflow-hidden transition-all duration-500 ease-in-out group-hover:scale-[120%] group-hover:rotate-[-2deg]"
                          alt={post.node.title}
                    ...
                    <div key={index} className="mt-8">
                      {post.node.featuredImage ? (
                        <div className="w-full h-[260px] overflow-hidden">
                          <Image
                            className="object-cover w-full h-[260px] overflow-hidden transition-all duration-500 ease-in-out group-hover:scale-[120%] group-hover:rotate-[-2deg]"
                            alt={post.node.title}
                            src={post.node.featuredImage.node.sourceUrl}
                            width={414}
                            height={260}
          ...
          {posts.map((post, index) => (
            <>
              {index !== 0 && index !== 1 && index !== 2 ? (
                <Link
                  href={"/nieuws/" + post.node.slug}
                  className="group w-1/3 no-underline pr-10 last:pr-0"
                >
                  <div key={index} className="mt-8">
                    {post.node.featuredImage ? (
                      <div className="w-full h-[260px] overflow-hidden">
